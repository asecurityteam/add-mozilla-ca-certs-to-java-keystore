...
def setup_args(parser=None):
    if parser is None:
        parser = argparse.ArgumentParser(
            '''A script to add mozilla's'''
            ' trusted CA certs to a java keystore.')
    parser.add_argument('-u', '--url', dest='mozilla_cert_url',
                        help='Url to the mozilla certdata.txt file. '
                        'Defaults to using certdata.txt from hg.mozilla.org.',
                        default='https://hg.mozilla.org/mozilla-central/'
                        'raw-file/tip/security/nss/lib/ckfw/builtins/'
                        'certdata.txt'
                        )
...
def main(args):
    go_extract_script_url = ('https://raw.github.com/agl/'
